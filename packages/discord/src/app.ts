	...
	if (commandName === 'globalping') {
		...
		try {
			...
			if (flags.cmd === 'help') {
				...
				const measurements = await postMeasurement(
				...
				for (const measurement of measurements) {
					...
					await interaction.editReply(
						`${userMention(user.id)}, here are the results for ${inlineCode(
							txtCommand
						)}`
					);
			...
			await interaction.editReply(
				`${userMention(
					user.id
				)}, there was an error processing your request for ${inlineCode(
					txtCommand ?? 'help'
				)}`
			);
