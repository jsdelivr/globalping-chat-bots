...
} from '@globalping/bot-utils/src/types';
...
const choiceMap = (arr: string[]) =>
	arr.map((item) => ({ name: item, value: item }));
	...
	new SlashCommandBuilder()
		...
		.addSubcommand((subcommand) =>
			subcommand
				...
				.addStringOption((option) =>
					option.setName('path').setDescription('Pathname').setRequired(false)
				)
				.addStringOption((option) =>
					option.setName('query').setDescription('URL query').setRequired(false)
				)
				.addStringOption((option) =>
					option.setName('host').setDescription('Hostname').setRequired(false)
				)
				.addStringOption((option) =>
					option.setName('header').setDescription('Headers').setRequired(false)
				)
		)
		.addSubcommand((subcommand) =>
			subcommand
